...
export function VersionControls({ flowId, onVersionChange }: VersionControlsProps) {
  ...
  const handlePublish = async (version: string) => {
    ...
    setLoading(true);
    try {
      const response = await apiClient.post(`/api/v1/flows/${flowId}/versions/${version}/publish`);
      if (response.data) {
        await loadVersions();
            ...
            {versions.map(v => (
              ...
              >
                <span className="text-sm">
                  v{v.version}
                  {v.published && (
                    <span className="ml-2 rounded bg-live-tint px-1 text-xs text-live-ink">
                      Published
                    </span>
                  )}
                </span>
                ...
                <Button size="sm" variant="ghost" onClick={() => handleShowDiff(v.version)}>
