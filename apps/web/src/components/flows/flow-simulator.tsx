...
export function FlowSimulator({ nodes, edges, onClose }: FlowSimulatorProps) {
  ...
  const handleStartSimulation = useCallback(() => {
    ...
    if (!firstNodeId) {
      toast({
        title: 'Error',
        description: 'No target node found from entry',
        variant: 'destructive',
      });
      return;
    }
              ...
              {simulationState.path.map((nodeId, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {index < simulationState.path.length - 1 && <span className="ml-2">→</span>}
                  </div>
                );
              })}
            </div>
            ...
            <div className="max-h-48 space-y-1 overflow-y-auto">
              {simulationState.events.map((event, index) => (
                <div key={index} className="text-xs">
                  ...
                  {event.data ? (
                    <span className="ml-2 text-muted-foreground">{JSON.stringify(event.data)}</span>
                  ) : null}
                </div>
              ))}
            </div>
  ...
  switch (currentNode.type) {
    case 'ivr': {
      if (eventType === 'dtmf.received') {
        ...
        const matchingEdge = outgoingEdges.find(e => e.data?.condition === digits);
    ...
    case 'if': {
      ...
      return result
        ? thenEdge?.target || (config.then as string) || null
        : elseEdge?.target || (config.else as string) || null;
    }

    case 'queue': {
      if (eventType === 'queue.connected') {
        return (
          (config.onConnect as string) ||
          outgoingEdges.find(e => e.data?.condition === 'connected')?.target ||
          null
        );
      }
      if (eventType === 'queue.timeout') {
        return (
          (config.onTimeout as string) ||
          outgoingEdges.find(e => e.data?.condition === 'timeout')?.target ||
          null
        );
      }
      break;
    ...
    case 'whisper': {
      if (eventType === 'whisper.accepted') {
        return (
          (config.onAccept as string) ||
          outgoingEdges.find(e => e.data?.condition === 'accept')?.target ||
          null
        );
      }
      if (eventType === 'whisper.rejected') {
        return (
          (config.onReject as string) ||
          outgoingEdges.find(e => e.data?.condition === 'reject')?.target ||
          null
        );
      }
      break;
    ...
    case 'record': {
      if (eventType === 'recording.completed') {
        return (
          (config.onComplete as string) ||
          outgoingEdges.find(e => e.data?.condition === 'complete')?.target ||
          null
        );
      }
      break;
