...
function BuyersPage() {
  ...
  /*
   * `?id=` opens that buyer, so a link from Today or Revenue lands on the
   * buyer it named rather than on the top of the list. Read once, after
   * mount, from `window.location`, so the page needs no Suspense boundary.
   */
  useEffect(() => {
    const requested = new URLSearchParams(window.location.search).get('id');
    if (requested) setExpandedBuyerId(requested);
  }, []);
                        ...
                        <TableRow key={`${buyer.id}-targets`}>
                          <TableCell colSpan={12} className="p-0 bg-sunken">
                            <div className="p-4">
                              ...
                              {/* Who from this buyer can sign in to the buyer portal. */}
