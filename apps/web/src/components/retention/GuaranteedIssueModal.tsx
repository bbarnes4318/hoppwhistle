    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div
        className={cn(
          ...
          'bg-surface',
          'rounded-card border border-dropped/40 shadow-sm ',
          'overflow-hidden animate-in fade-in-0 zoom-in-95 duration-200'
        )}
      >
        {/* Header */}
        <div className="px-6 py-4 border-b border-dropped/40 bg-dropped-tint">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-xl bg-dropped flex items-center justify-center animate-pulse">
                <AlertTriangle className="w-6 h-6 text-white" />
              </div>
              <div>
                <h2 className="text-ink text-lg font-semibold">Guaranteed Issue Offer Required</h2>
                <p className="text-dropped-ink text-sm">
                  Policy declined - Read GI script to customer
                </p>
              </div>
            </div>
            <Button
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-6 text-sm">
              <div className="text-center">
                <p className="text-ink-3">Original Coverage</p>
                <p className="text-ink font-semibold">{formatCurrency(policy.coverage || 0)}</p>
              </div>
              <div className="text-center">
                <p className="text-ink-3">Carrier</p>
                <p className="text-ink font-semibold">{policy.carrier || 'N/A'}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 py-4 max-h-[300px] overflow-y-auto">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h4 className="text-ink font-medium">GI Sales Script</h4>
          </div>
          <div className="p-4 rounded-lg bg-sunken border border-rule">
            <p className="text-ink-2 text-sm whitespace-pre-line leading-relaxed">
              {personalizedScript}
            </p>
          </div>
        </div>
        ...
        <div className="px-6 py-4 border-t border-rule">
          <label className="text-ink-3 text-sm mb-2 block">Call Notes</label>
          <Textarea
            value={notes}
        ...
        <div className="px-6 py-4 border-t border-rule flex items-center justify-between gap-4">
          <Button variant="outline" onClick={onClose} className="border-rule">
            Cancel
          </Button>
          <div className="flex items-center gap-3">
            <Button
