            ...
            <DialogContent className="max-w-md max-h-[80vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-3 py-4">
                {ALL_COLUMNS.map(column => (
                  <label
                    key={column.key}
                    className="flex items-center gap-2 cursor-pointer hover:bg-sunken p-2 rounded-md"
                  >
                    <Checkbox
                ...
                <TableBody>
                  {calls.map(call => (
                    <TableRow
                      ...
                      className={cn(
                        'group',
                        call.missedCall && 'bg-dropped-tint',
                        call.converted && !call.missedCall && 'bg-live-tint'
                      )}
                    >
                      {ALL_COLUMNS.filter(c => visibleColumns.includes(c.key)).map(column => (
