...
export function UpfrontBuyerBalances() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'ACTIVE':
        return (
          <Badge variant="outline" className="bg-live-tint text-live-ink">
            Active
          </Badge>
        );
      case 'PAUSED':
        return (
          <Badge variant="outline" className="bg-ringing-tint text-ringing-ink">
            Paused
          </Badge>
        );
      case 'INACTIVE':
              ...
              <TableBody>
                {balances.map(buyer => (
                  <TableRow
                    key={buyer.id}
                    className={cn(
                      buyer.isLowBalance && 'bg-dropped-tint hover:bg-dropped-tint',
                      buyer.status === 'PAUSED' && 'bg-ringing-tint'
                    )}
                  >
                    ...
                    <TableCell
                      className={cn(
                        ...
                        buyer.isLowBalance && 'text-dropped-ink',
                        buyer.leadsRemaining === 0 && 'text-dropped-ink'
                      )}
                    >
