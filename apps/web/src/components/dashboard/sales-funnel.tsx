...
export function SalesFunnel({ onStageClick }: SalesFunnelProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setStages([
        { label: 'Total Calls', value: 0, percentage: 100, color: 'bg-ink-3' },
        { label: 'Qualified', value: 0, percentage: 0, color: 'bg-money' },
        { label: 'Billable', value: 0, percentage: 0, color: 'bg-live' },
        { label: 'Sold', value: 0, percentage: 0, color: 'bg-brand' },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Card>
      ...
      <CardContent>
        {error && (
          <div className="mb-4 rounded-lg border border-ringing/40 bg-ringing-tint p-2 text-xs text-ringing-ink">
            {error}
          </div>
        )}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {getDropOff(index) && (
                      <span className="flex items-center gap-1 text-xs text-muted-foreground">
                        ...
                        <span className="text-dropped-ink">-{getDropOff(index)}%</span>
                      </span>
                    )}
                  </div>
