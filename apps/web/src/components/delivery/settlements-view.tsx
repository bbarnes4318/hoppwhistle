...
function settlementMode(
  status: string,
  /** Who the agency deals with: NetEnroll, or on a branded portal its account manager. */
  contact = 'NetEnroll'
  ...
  switch (status) {
    ...
    case 'HALTED_MAX_DEBIT':
      return {
        label: 'halted',
        detail: `The total came out above the maximum daily debit on your Insertion Order, so no payment was taken and ${contact} was alerted.`,
      };
              ...
              <tbody>
                {rows.map(row => {
                  ...
                  const mode = settlementMode(
                    row.paymentStatus,
                    brand ? 'your account manager' : 'NetEnroll'
                  );
                  return (
                    <Fragment key={row.id}>
                      <tr
            ...
            <>
              ...
              {derivation.matchesStoredRate ? (
                ...
                <p className="mt-2 t-meta text-ringing-ink">
                  ...
                  means the underlying calls or applications for those days have moved since.{' '}
                  {brand
                    ? 'Please contact your account manager and they will go through it with you.'
                    : 'Please contact NetEnroll and we will go through it with you.'}
                </p>
              )}
            </>
          )}
 ...
 */
export function SettlementsView(): JSX.Element {
  return (
