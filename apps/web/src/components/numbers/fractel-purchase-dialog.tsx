...
interface FractelPurchaseDialogProps {
  open: boolean;
...
export function FractelPurchaseDialog({
  open,
  onOpenChange,
  onSuccess,
}: FractelPurchaseDialogProps) {
  ...
  const [numbers, setNumbers] = useState<FractelNumber[]>([]);
  const [selectedNumber, setSelectedNumber] = useState<FractelNumber | null>(null);
  ...
  const handleSearch = async () => {
    ...
    setNumbers([]);
    try {
      const params = new URLSearchParams({ type: numberType });
      ...
      }>(`/api/v1/fractel/available?${params.toString()}`);
  ...
  const handleSelect = (num: FractelNumber) => {
        ...
        return (
          <div className="space-y-4">
            ...
            <div className="text-sm text-muted-foreground">
              {numberType === 'local'
                ? 'Enter a 3-digit area code to search available local numbers.'
                : 'Search available toll-free numbers. Optionally enter a prefix (800, 888, 877…).'}
            </div>
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                ...
                <Input
                  placeholder={
                    numberType === 'local' ? 'Area code (e.g. 310)' : 'Prefix (optional, e.g. 888)'
                  }
                  value={areaCode}
            ...
            ) : (areaCode.length === 3 || numberType === 'tollfree') && !error && !loading ? (
              <div className="text-center py-8 text-muted-foreground">No numbers available.</div>
            ) : null}
          </div>
        );
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {selectedNumber?.metadata?.rateCenter
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>
            {step === 'success' ? 'Number Added' : 'Add Phone Number (FracTEL)'}
          </DialogTitle>
          <DialogDescription>
            {step === 'success'
              ? 'Your new number is ready to use'
              : 'Search and order local or toll-free numbers from FracTEL.'}
          </DialogDescription>
        </DialogHeader>
