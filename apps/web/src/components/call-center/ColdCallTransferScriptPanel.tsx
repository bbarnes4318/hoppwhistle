...
export function ColdCallTransferScriptPanel({
  ...
  const currentState =
    prospectData?.state || prospectData?.stateCode || customState || 'your state';
  ...
  const interpolatedText = useMemo(() => {
    ...
    return activeNode.text
      ...
      .replace(
        /{callback_time}/g,
        capturedData.callbackTime || customCallbackTime || 'scheduled time'
      )
  ...
  const generateNotesSummary = useCallback((): string => {
    ...
    if (capturedData.hasFinalExpenseCoverage)
      lines.push(`Has Coverage: ${capturedData.hasFinalExpenseCoverage}`);
    ...
    if (capturedData.correctState || prospectData?.state)
      lines.push(`State: ${capturedData.correctState || prospectData?.state}`);
    if (capturedData.tobaccoStatus)
      lines.push(
        `Tobacco Nicotine: ${capturedData.tobaccoStatus} (${capturedData.tobaccoType || 'N/A'})`
      );
    if (capturedData.majorHealthHistory)
      lines.push(
        `2-Yr Major Health: ${capturedData.majorHealthHistory} (${capturedData.majorHealthDetails || 'None'})`
      );
    ...
    if (licensedAgentName && activeStepId === 'warm_transfer_intro')
      lines.push(`Transferred to Agent: ${licensedAgentName}`);
  ...
  const suggestedDisposition = useMemo((): { disp: string; label: string } | null => {
    ...
    if (activeStepId === 'wrong_number_exit')
      return { disp: 'WRONG_NUMBER', label: 'Wrong Number' };
    if (activeStepId === 'dnc_exit')
      return { disp: 'NOT_INTERESTED', label: 'Not Interested (DNC)' };
    if (
      activeStepId === 'under_age_exit' ||
      activeStepId === 'over_age_exit' ||
      activeStepId === 'state_not_available_exit'
    ) {
    ...
    if (activeStepId === 'end_call_after_transfer')
      ...
      <div className="flex-1 flex flex-col p-5 overflow-y-auto border-r border-rule space-y-4">
          ...
          <div className="mt-4 pt-4 border-t border-rule">
            {activeStepId === 'correct_state' && (
              <div className="flex items-center gap-2 max-w-sm">
                <input
                  ...
                  className="bg-surface border border-rule rounded px-2.5 py-1.5 text-xs font-mono outline-none text-ink flex-1 focus:border-brand-ink"
                />
                <button
            ...
            {activeStepId === 'major_health_details' && (
              <div className="flex flex-col gap-2 max-w-md">
                <textarea
                  ...
                  rows={2}
                  className="bg-surface border border-rule rounded p-2 text-xs font-mono outline-none text-ink focus:border-brand-ink resize-none"
                />
                <button
                  ...
                  className="bg-brand self-end px-3 py-1.5 text-xs text-ink font-mono rounded disabled:opacity-50"
                >
                  Save & Continue
                </button>
              </div>
            ...
            {activeStepId === 'warm_transfer_intro' && (
              <div className="flex items-center gap-2 max-w-sm mb-3">
                <label className="text-[10px] font-mono text-ink-2 uppercase">Agent Name:</label>
                <input
                  type="text"
                  value={licensedAgentName}
                  onChange={e => setLicensedAgentName(e.target.value)}
                  placeholder="Licensed Agent Name..."
                  className="bg-surface border border-rule rounded px-2.5 py-1 text-xs font-mono outline-none text-ink focus:border-brand-ink"
                />
              </div>
            )}
            ...
            {activeNode.options &&
              ...
              !['correct_state', 'major_health_details', 'schedule_callback'].includes(
                activeStepId
              ) && (
                <div className="flex flex-wrap gap-2.5">
                  {Object.entries(activeNode.options).map(([label, targetId]) => {
                    const isPositive =
                      label.includes('Yes') ||
                      label.includes('Continues') ||
                      label.includes('Agree');
                    const isNegative =
                      label.includes('No') ||
                      label.includes('Decline') ||
                      label.includes('Refuses');
                    return (
                      <button
                        key={label}
                        onClick={() => {
        ...
        <div className="bg-ringing-tint border border-ringing rounded-card p-4 space-y-2">
          ...
          <ul className="text-[11px] text-ringing-ink space-y-1 font-sans leading-relaxed list-disc pl-4">
            ...
            <li>
              <strong>Do not</strong> collect sensitive SSN, banking, routing, or account numbers.
            </li>
            <li>
              <strong>Do not</strong> say approved, qualified, guaranteed, government, free, or
              state-approved.
            </li>
            <li>
              You are screening and transferring to a licensed agent, <strong>NOT</strong> selling
              the policy.
            </li>
          </ul>
        </div>
      ...
      <div className="w-full lg:w-72 bg-sunken p-5 flex flex-col space-y-4">
        ...
        <div className="flex-1 overflow-y-auto space-y-2.5 font-mono text-[11px] max-h-[400px] lg:max-h-none">
          <div className="flex items-center justify-between border-b border-rule pb-1">
            <span className="text-ink-2 uppercase">Age Range:</span>
            <span className="text-ink">{capturedData.ageRange || '—'}</span>
          </div>
          <div className="flex items-center justify-between border-b border-rule pb-1">
            <span className="text-ink-2 uppercase">Has Coverage:</span>
            <span className="text-ink">{capturedData.hasFinalExpenseCoverage || '—'}</span>
          </div>
          <div className="flex items-center justify-between border-b border-rule pb-1">
            <span className="text-ink-2 uppercase">Coverage Type:</span>
            <span className="text-ink">{capturedData.coverageType || '—'}</span>
          </div>
          <div className="flex items-center justify-between border-b border-rule pb-1">
            <span className="text-ink-2 uppercase">Arranger:</span>
            <span className="text-ink">{capturedData.responsiblePerson || '—'}</span>
          </div>
          <div className="flex items-center justify-between border-b border-rule pb-1">
            <span className="text-ink-2 uppercase">Burden Pain:</span>
            <span className="text-ink">{capturedData.financialBurden || '—'}</span>
          </div>
          <div className="flex items-center justify-between border-b border-rule pb-1">
            <span className="text-ink-2 uppercase">State:</span>
            <span className="text-ink">
              {capturedData.correctState || prospectData?.state || '—'}
            </span>
          </div>
          <div className="flex items-center justify-between border-b border-rule pb-1">
            <span className="text-ink-2 uppercase">Tobacco Status:</span>
            <span className="text-ink">{capturedData.tobaccoStatus || '—'}</span>
          </div>
          {capturedData.tobaccoType && (
            <div className="flex items-center justify-between border-b border-rule pb-1 pl-2">
              <span className="text-ink-2 uppercase">Tobacco Type:</span>
              <span className="text-ink">{capturedData.tobaccoType}</span>
            </div>
          )}
          <div className="flex items-center justify-between border-b border-rule pb-1">
            <span className="text-ink-2 uppercase">Health History:</span>
            <span className="text-ink">{capturedData.majorHealthHistory || '—'}</span>
          </div>
          {capturedData.majorHealthDetails && (
            <div className="flex flex-col border-b border-rule pb-1 pl-2 gap-0.5">
              <span className="text-ink-2 uppercase">Health Details:</span>
              <span className="text-ink-2 break-words">{capturedData.majorHealthDetails}</span>
            </div>
          )}
          <div className="flex items-center justify-between border-b border-rule pb-1">
            <span className="text-ink-2 uppercase">Bank Account:</span>
            <span className="text-ink">{capturedData.hasBankAccount || '—'}</span>
          </div>
          {capturedData.callbackTime && (
            <div className="flex flex-col border-b border-rule pb-1 gap-0.5">
              ...
              <span className="text-live-ink">{capturedData.callbackTime}</span>
            </div>
          )}
        </div>
