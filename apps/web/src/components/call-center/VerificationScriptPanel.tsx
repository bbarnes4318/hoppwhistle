...
export function VerificationScriptPanel({
  ...
  const customerName =
    `${prospectData?.firstName || prospectData?.first_name || 'Customer'} ${prospectData?.lastName || prospectData?.last_name || ''}`.trim();
  const customerPhone = prospectData?.phone || prospectData?.caller_id || '';
  const customerDOB = prospectData?.dob || '[Date of Birth]';
  const carrierName =
    prospectData?.selectedCarrier ||
    prospectData?.carrier ||
    prospectData?.carrierName ||
    '[Carrier Name]';
  const coverageAmount =
    prospectData?.selectedCoverage ||
    prospectData?.coverageAmount ||
    prospectData?.replacementFaceAmount ||
    prospectData?.faceAmount ||
    prospectData?.coverage_amount ||
    '[Face Amount]';
  const monthlyPremium =
    prospectData?.selectedPremium ||
    prospectData?.monthlyPremium ||
    prospectData?.premium ||
    prospectData?.monthly_premium ||
    '[Premium]';
  const requestedEffectiveDate =
    prospectData?.requestedEffectiveDate ||
    prospectData?.effectiveDate ||
    prospectData?.effective_date ||
    '[Requested Effective Date]';
  const draftDay =
    prospectData?.draftDay || prospectData?.draftDate || prospectData?.draft_day || '[Draft Day]';
  const beneficiaryName =
    prospectData?.beneficiaryName ||
    prospectData?.primaryBeneficiaryName ||
    prospectData?.primaryBenName ||
    prospectData?.beneficiary ||
    '[Beneficiary Name]';
  const beneficiaryRelationship =
    prospectData?.beneficiaryRelation ||
    prospectData?.primaryBeneficiaryRelationship ||
    prospectData?.primaryBenRel ||
    prospectData?.beneficiary_relationship ||
    '[Relationship]';
  ...
  const handleLogDisposition = useCallback(
    (disposition: string, autoNote?: string) => {
...
Customer must answer clearly.`,
        stageDirection:
          'Listen carefully. If yes, proceed to confirm understanding. If no/confused, go to pause flow.',
        options: [
...
"Great. In your own words, can you tell me what you understood you were applying for?"`,
        stageDirection:
          'Listen for words like: life insurance, burial insurance, final expense, whole life, coverage for funeral expenses.',
        options: [
          {
            label: 'Understands coverage details -> Continue',
            nextStep: 'authorized_app',
            color: 'emerald',
          },
          {
            label: 'Seems confused / No memory -> Abort',
...
"No problem. I’m going to pause this application and send it for review. We do not want anything processed unless you clearly remember and understand it."`,
        stageDirection:
          'Flag file and log disposition as NO MEMORY / CONFUSED or DENIES APPLICATION.',
        options: [
          {
            ...
            action: () =>
              handleLogDisposition(
                'NO_MEMORY_CONFUSED',
                'Customer does not clearly remember applying or cannot explain what happened.'
              ),
          },
          {
            ...
            action: () =>
              handleLogDisposition(
                'DENIES_APPLICATION',
                'Customer says they did not apply or did not authorize it.'
              ),
          },
        ],
        isEndNode: true,
...
(Expected answer: No)"`,
        stageDirection:
          'If they say yes to pressure, are unclear, or hesitate heavily, route to Abort flow and flag.',
        options: [
          {
            label: 'Permission granted & No pressure -> Continue',
            nextStep: 'policy_details',
            color: 'emerald',
          },
          {
            label: 'Suspected Pressure / Coaching -> Abort',
            nextStep: 'abort_wrapup',
            action: () =>
              handleLogDisposition(
                'POSSIBLE_PRESSURE_COACHING',
                'Customer says they were rushed, pressured, or told what to say.'
              ),
          },
          {
            label: 'No permission / Denies -> Abort',
            nextStep: 'abort_wrapup',
            action: () =>
              handleLogDisposition(
                'DENIES_APPLICATION',
                'Customer did not personally authorize application.'
              ),
          },
        ],
      },
...
(Expected answer: Yes)`,
        stageDirection:
          'Confirm details match exactly. If they disagree, stop and send for review.',
        options: [
        ...
        options: [
          {
            ...
            action: () =>
              handleLogDisposition(
                ...
                `Customer disagrees with premium ($${monthlyPremium}), coverage ($${coverageAmount}), carrier (${carrierName}), or effective date (${requestedEffectiveDate}).`
              ),
          },
        ],
        isEndNode: true,
...
(Expected answer: Yes)"`,
        ...
        options: [
          {
            label: 'Payment Authorized -> Continue',
            nextStep: 'confirm_beneficiary',
            color: 'emerald',
          },
          {
            label: 'Payment Issue / Authorization Denied',
        ...
        options: [
          {
            ...
            action: () =>
              handleLogDisposition(
                'PAYMENT_ISSUE',
                'Customer did not understand or authorize bank draft.'
              ),
          },
        ],
        isEndNode: true,
...
(Expected answer: Yes)`,
        stageDirection:
          'If wrong, note correct details in call notes and flag for update, but you may proceed to health questions.',
        options: [
          { label: 'Correct -> Continue', nextStep: 'health_answers', color: 'emerald' },
          {
            label: 'Incorrect Beneficiary details',
            nextStep: 'confirm_beneficiary_no',
            color: 'amber',
          },
        ],
      },
...
"Thank you. I have made a note of the correct beneficiary details so we can update the policy. Let's move to the last few questions."`,
        stageDirection:
          'Instruct the agent to write the correct beneficiary name/relationship in the call notes on the right.',
        options: [
...
(Expected answer: Yes)"`,
        stageDirection:
          'Ensure they confirm answers were accurate. If not, flag policy for review.',
        options: [
          {
            label: 'Yes, truthful & personal -> Continue',
            nextStep: 'final_confirmation',
            color: 'emerald',
          },
          {
            label: 'No / Untruthful or Not Personal -> Abort',
        ...
        options: [
          {
            ...
            action: () =>
              handleLogDisposition(
                'POLICY_DETAILS_WRONG',
                'Customer claims health answers were not theirs or were inaccurate.'
              ),
          },
        ],
        isEndNode: true,
        ...
        options: [
          {
            ...
            action: () =>
              handleLogDisposition(
                'VERIFIED',
                'Customer remembers the call, confirms application, confirms premium/payment, and wants to proceed.'
              ),
          },
          {
            label: 'Customer Denies at Last Moment',
            nextStep: 'abort_wrapup',
            action: () =>
              handleLogDisposition(
                'DENIES_APPLICATION',
                'Customer decided not to continue application at final confirmation.'
              ),
          },
        ],
        isEndNode: true,
...
"Thank you for letting me know. I’m going to pause this application immediately and send it for review. We do not want anything submitted or processed unless you clearly understand it and personally authorized it."`,
        stageDirection:
          'Do not argue. Do not explain too much. Do not try to save the sale. End call and select appropriate disposition.',
        options: [
          {
            label: 'Log No Memory / Confused',
            nextStep: 'abort_wrapup',
          ...
          },
          {
            ...
            action: () =>
              handleLogDisposition(
                'POLICY_DETAILS_WRONG',
                'Customer disagrees with premium, coverage, carrier, beneficiary, or draft date.'
              ),
          },
          {
  ...
  return (
    <div className="h-full flex flex-col overflow-auto bg-surface text-ink border border-rule rounded-card">
      {/* Header Banner */}
      <div className="bg-sunken border-b border-rule p-4 flex-shrink-0">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-live-tint rounded-card flex items-center justify-center">
              <ShieldCheck className="w-5 h-5 text-live-ink" />
            </div>
            <div>
              <h2 className="text-sm font-bold text-live-ink uppercase tracking-wider">
          ...
          <span className="text-xs font-mono font-bold text-live-ink bg-live-tint border border-live px-2 py-1 rounded">
      ...
      {/* Customer Info strip */}
      {customerName && (
        <div className="bg-sunken border-b border-rule px-4 py-2 flex flex-wrap gap-4 text-xs font-mono text-ink-2">
          <div className="flex items-center gap-1.5">
            <User className="w-3.5 h-3.5 text-live-ink" />
            <span className="text-ink font-medium">{customerName}</span>
          </div>
          {customerDOB && (
            <div className="flex items-center gap-1">
              <span>DOB:</span>
              <span className="text-ink font-medium">{customerDOB}</span>
            </div>
          )}
          {customerPhone && (
            <div className="flex items-center gap-1">
              <Phone className="w-3 h-3 text-live-ink" />
          ...
          {carrierName && (
            <div className="flex items-center gap-1 border-l border-rule pl-3">
              <span>Carrier:</span>
              <span className="text-ink font-medium">{carrierName}</span>
            </div>
          )}
          {coverageAmount && (
            <div className="flex items-center gap-1">
              <span>Coverage:</span>
              <span className="text-ink font-medium">${coverageAmount}</span>
            </div>
          )}
          {monthlyPremium && (
            <div className="flex items-center gap-1">
              <span>Premium:</span>
              <span className="text-ink font-medium">${monthlyPremium}/mo</span>
            </div>
          )}
        </div>
      )}
      ...
      <div className="flex-1 p-5 overflow-auto space-y-4">
        <div className="bg-surface border border-rule rounded-card p-5 space-y-4">
          <div className="flex items-start gap-3">
            <div className="w-8 h-8 bg-live-tint rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5">
              <HelpCircle className="w-4 h-4 text-live-ink" />
            </div>
            <div>
          ...
          {/* Script Text */}
          <div className="bg-sunken border border-rule rounded-lg p-4 font-sans text-sm text-ink whitespace-pre-wrap leading-relaxed">
            {currentStep.script}
          </div>

          {/* Stage Direction */}
          {currentStep.stageDirection && (
            <div className="flex items-start gap-2 p-3 bg-ringing-tint rounded-lg border border-ringing text-xs text-ringing-ink">
          ...
          {currentStep.options && currentStep.options.length > 0 && (
            <div className="space-y-3 pt-2">
              <p className="text-[10px] text-ink-2 uppercase tracking-wider font-mono">
              ...
              <div className="grid gap-2">
                {currentStep.options.map((option, idx) => {
                  const colorClasses = {
                    ...
                    blue: 'bg-live-tint border-live hover:bg-brand hover:text-ink text-live-ink',
                    amber:
                      'bg-ringing-tint border-ringing hover:bg-brand hover:text-ink text-ringing-ink',
                    orange: 'bg-ringing-tint border-ringing hover:bg-ringing-tint text-ringing-ink',
                  };
                  ...
                  return (
                    <Button
                      ...
                      className={cn(
                        'justify-start text-left w-full border font-mono text-xs uppercase tracking-wider py-5 transition-all',
                        buttonClass
                      )}
                    >
                      {option.label}
          ...
          {activeStepId !== 'abort_wrapup' && !currentStep.isEndNode && (
            <div className="pt-2 border-t border-rule mt-4">
              <Button
                ...
                className="w-full bg-dropped-tint border-dropped text-dropped-ink hover:bg-dropped-tint font-mono text-xs uppercase tracking-wider"
              >
      ...
      <div className="flex items-center justify-between p-4 border-t border-rule bg-surface flex-shrink-0">
        <Button
          ...
          className="border-rule text-ink-2 hover:bg-sunken hover:text-ink font-mono text-xs uppercase tracking-widest"
        >
