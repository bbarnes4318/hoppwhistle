...
export function ContextPanel({ currentStep }: ContextPanelProps) {
  ...
  if (isCollapsed) {
    return (
      <div className="flex flex-col items-center py-4 px-2 border-l bg-sunken">
  ...
  return (
    <div className="w-80 border-l bg-sunken flex flex-col">
      {/* Header */}
        ...
        <div className="space-y-3">
          {context.tips.map((tip, index) => (
            <div key={index} className="flex gap-3 text-sm">
              <Lightbulb className="h-4 w-4 text-ringing-ink shrink-0 mt-0.5" />
        ...
        {context.warnings && context.warnings.length > 0 && (
          <div className="pt-4 border-t space-y-3">
            {context.warnings.map((warning, index) => (
              <div key={index} className="flex gap-3 text-sm">
                <AlertCircle className="h-4 w-4 text-ringing-ink shrink-0 mt-0.5" />
                <span className="text-ringing-ink">{warning}</span>
              </div>
            ))}
          </div>
        )}
