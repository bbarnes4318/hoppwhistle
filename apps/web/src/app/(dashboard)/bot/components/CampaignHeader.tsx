...
export function CampaignHeader({
  ...
  const getStatusBadge = () => {
    ...
    > = {
      ...
      running: {
        variant: 'default',
        label: 'Running',
        className: 'bg-ringing-tint text-ringing-ink animate-pulse',
      },
      paused: { variant: 'outline', label: 'Paused', className: 'border-ringing text-ringing-ink' },
      complete: { variant: 'default', label: 'Complete', className: 'bg-live-tint text-live-ink' },
      error: { variant: 'destructive', label: 'Error' },
    };

    // Override idle status if ready
    if (status === 'idle' && isReady) {
      return <Badge className="bg-live-tint text-live-ink">Ready</Badge>;
    }
    ...
    switch (status) {
      ...
      case 'paused':
        return (
          <Button
            onClick={onStart}
            className="gap-2 bg-brand text-brand-fg hover:bg-brand-ink hover:text-surface"
          >
      ...
      default:
        return (
          <Button
            onClick={onStart}
            disabled={!isReady}
            className="gap-2 bg-brand text-brand-fg hover:bg-brand-ink hover:text-surface disabled:opacity-50"
          >
            <Play className="h-4 w-4" />
  ...
  return (
    <div className="sticky top-0 z-50 border-b bg-background/95 supports-[backdrop-filter]:bg-background/60">
