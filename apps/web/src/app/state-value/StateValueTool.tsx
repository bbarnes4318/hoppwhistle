...
type SortKey = 'valueIndex' | 'name' | 'region' | 'population' | 'seniors' | 'totalFee' | 'costPer1k';
...
export function StateValueTool() {
  ...
  function toggleSort(key: SortKey) {
    if (key === sortKey) {
      ...
      setSortDir(key === 'name' || key === 'region' || key === 'costPer1k' || key === 'totalFee' ? 'asc' : 'desc');
    }
  }
  ...
  const columns: { key: SortKey; label: string; align: 'left' | 'right'; hint: string }[] = [
    ...
    { key: 'valueIndex', label: 'Value index', align: 'right', hint: 'Seniors reached per dollar, best state = 100' },
  ];
    ...
    <div className="flex h-[100dvh] flex-col overflow-hidden bg-[#070913] font-sans text-white selection:bg-emerald-500/30">
        ...
        <div className="flex items-center gap-3">
          <Link href="/" className="group flex items-center gap-2.5">
            <div className="relative h-7 w-7 overflow-hidden rounded-lg border border-slate-800 bg-slate-900 p-1 transition-all duration-300 group-hover:border-emerald-500/30">
              <Image src="/hopwhistle.png" alt="NetEnroll logo" fill className="object-contain p-1" priority />
            </div>
          ...
          <div className="hidden sm:block">
            <h1 className="text-sm font-medium leading-tight tracking-tight text-white">State Value Evaluator</h1>
      ...
      <div className="flex shrink-0 flex-wrap items-center gap-2 border-b border-slate-900 bg-slate-950/40 px-4 py-2 md:px-6">
        <label className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-wider text-slate-500">
          Resident state
          <select value={residence} onChange={(e) => setResidence(e.target.value)} className={controlClass}>
            {[...STATES]
      ...
      <div className="flex min-h-0 flex-1 gap-3 p-3 md:px-6 md:py-3">
        <main className="flex min-h-0 min-w-0 flex-1 flex-col gap-3">
          ...
          <div className="grid shrink-0 grid-cols-2 gap-2 lg:grid-cols-4">
            {[
              { label: 'Best value', value: best ? best.name : '—', sub: best ? `index ${fmtIndex(best.valueIndex)}` : '' },
              {
                label: 'Lowest cost / 1K',
                value: best ? `$${best.costPer1k.toFixed(4)}` : '—',
                sub: `median $${medianCost.toFixed(4)}`,
              },
              { label: 'Jurisdictions shown', value: String(visible.length), sub: `of ${scored.length}` },
              {
                label: 'Plan cost',
                value: money.format(plan.cost),
                sub: plan.rows.length ? `${compactNum(plan.seniors)} seniors reached` : 'select rows to build',
              },
            ].map((kpi) => (
              <div key={kpi.label} className="rounded-lg border border-slate-800/80 bg-slate-900/40 px-3 py-2">
                <div className="text-[10px] font-medium uppercase tracking-wider text-slate-500">{kpi.label}</div>
                <div className="truncate font-mono text-base font-medium tabular-nums text-emerald-400">{kpi.value}</div>
                          ...
                          <div className="flex items-center justify-end gap-2">
                            <div className="h-1.5 w-20 overflow-hidden rounded-full bg-slate-800">
                              <div
                                ...
                                style={{ width: `${Math.max(2, (s.valueIndex / maxIndex) * 100)}%` }}
                              />
                            </div>
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-col items-center justify-center rounded border transition-transform duration-150 hover:scale-105 ${
                        isSelected ? 'ring-2 ring-emerald-400 ring-offset-1 ring-offset-[#070913]' : ''
                      }`}
                    >
                      <span className="font-mono text-[11px] font-semibold leading-none">{s.code}</span>
        ...
        <aside className="hidden w-64 shrink-0 flex-col gap-3 xl:flex">
          <div className="flex min-h-0 flex-1 flex-col rounded-lg border border-slate-800/80 bg-slate-900/40">
            <div className="shrink-0 border-b border-slate-800 px-3 py-2">
              <h2 className="text-[10px] font-medium uppercase tracking-wider text-slate-500">Licensing plan</h2>
            ...
            <div className="min-h-0 flex-1 overflow-auto">
              {plan.rows.length === 0 ? (
                <p className="px-3 py-4 text-[11px] leading-relaxed text-slate-600">
                  No states selected. Sort by value index and add the top jurisdictions to price out a
                  multi-state licensing run.
                </p>
              ) : (
                plan.rows.map((s) => (
                  <button
            ...
            <div className="shrink-0 space-y-1 border-t border-slate-800 px-3 py-2">
              ...
              <div className="flex items-baseline justify-between">
                ...
                <span className="font-mono text-xs tabular-nums text-slate-300">{compactNum(plan.seniors)}</span>
              </div>
              <div className="flex items-baseline justify-between">
          ...
          <div className="shrink-0 rounded-lg border border-slate-800/80 bg-slate-900/40 px-3 py-2">
            <div className="mb-1.5 text-[10px] font-medium uppercase tracking-wider text-slate-500">Value index</div>
            <div className="flex h-1.5 overflow-hidden rounded-full">
              {[0.1, 0.3, 0.5, 0.7, 0.9].map((t) => (
                <div key={t} className="flex-1" style={{ backgroundColor: `rgba(16, 185, 129, ${0.06 + t * 0.62})` }} />
              ))}
            </div>
      ...
      <footer className="flex h-8 shrink-0 items-center justify-between border-t border-slate-900 px-4 text-[11px] text-slate-600 md:px-6">
        <span>
          Value index = residents aged 55-80 reached per licensing dollar, indexed to the strongest jurisdiction.
        </span>
