...
| `TWILIO_API_KEY` / `TWILIO_API_SECRET`                                                                                                 | A      | CNAM/carrier lookup; also accepted as the provisioning API key pair | not on the call path itself                     |
| `TWILIO_ACCOUNT_SID` / `TWILIO_AUTH_TOKEN` / `TWILIO_TRUNK_SID` / `TWILIO_VOICE_URL`                                                   | A      | Twilio number provisioning                                          | a purchase needs a trunk SID or voice URL       |
| `TWILIO_SIP_TERMINATION_DOMAIN` / `TWILIO_SIP_USERNAME` / `TWILIO_SIP_PASSWORD`                                                        | FS     | `twilio` gateway (Elastic SIP Trunking)                             | domain is per-trunk `<trunk>.pstn.twilio.com`   |
| `VONAGE_API_KEY` / `VONAGE_API_SECRET` / `VONAGE_APPLICATION_ID` / `VONAGE_SIP_URI` / `VONAGE_DEFAULT_COUNTRY`                         | A      | Vonage number provisioning                                          | a purchase needs an app id or SIP URI           |
| `VONAGE_SIP_PROXY` / `VONAGE_SIP_REALM` / `VONAGE_SIP_USERNAME` / `VONAGE_SIP_PASSWORD`                                                | FS     | `vonage` gateway (SIP trunking)                                     | proxy defaults to `sip.nexmo.com`               |
