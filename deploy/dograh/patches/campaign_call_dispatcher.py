...
# HOPWHISTLE PATCH (state-matched caller ID): canonical NPA -> state mapping,
# mounted alongside this file (see docker-compose.override.yaml).
from api.services.campaign.areacode_state import (
    REASON_OK,
    state_for_number,
)
...
class NoCallerIdForDestinationStateError(Exception):
    ...
    the destination. The queued run is marked failed — the call is never bridged
    with an out-of-state caller ID."""
...
def resolve_state_cid_policy(campaign) -> str:
    """Per-campaign caller-ID state-matching policy.

    Order: campaign.orchestrator_metadata.state_cid_policy, then env
    DOGRAH_STATE_CID_POLICY, else "off".
      off    — original whole-pool rotation (default; kill switch).
      prefer — same-state caller ID when one exists in the pool, else any.
      strict — same-state only; fail the call with
               NO_CALLER_ID_AVAILABLE_FOR_DESTINATION_STATE when none exists.
    """
    policy = None
    om = campaign.orchestrator_metadata
    if isinstance(om, dict):
        policy = om.get("state_cid_policy")
    if not policy:
...
class CampaignCallDispatcher:
    ...
    async def dispatch_call(
        ...
        try:
            ...
            # HOPWHISTLE PATCH (state-matched caller ID): resolve the
            # destination's state and, per policy, restrict caller-ID selection
            # to same-state numbers from the pool. All selection stays
            # server-side and atomic (Redis Lua) — nothing client-supplied.
            ...
            if state_cid_policy != "off":
                ...
                same_state = (
                    [
                        ...
                        if state_for_number(n)[0] == dest_state
                    ]
                    if dest_state
                    else []
                )
                if same_state:
                    ...
                    logger.warning(
                        ...
                        f"reason={NO_CALLER_ID_FOR_STATE_REASON} "
                        f"destination={phone_number} destination_state={dest_state} "
                        f"state_reason={dest_state_reason} "
                        f"campaign_id={campaign.id} org_id={campaign.organization_id} "
                        f"pool_size={len(pool_numbers)}"
                    )
                    ...
                    logger.info(
                        ...
                        f"destination_state={dest_state} (reason={dest_state_reason}); "
                        f"falling back to whole pool. campaign_id={campaign.id}"
                    )
            ...
            # caller IDs across configs. When state matching is active, only
            # same-state numbers may be acquired.
            from_number = await self.acquire_from_number(
                campaign.organization_id,
                telephony_configuration_id=campaign.telephony_configuration_id,
                allowed_numbers=allowed_from_numbers,
                # prefer-policy: don't stall the batch waiting on a busy
                # same-state subset — fall back to the whole pool quickly.
                timeout=(
                    5
                    if (state_match_active and state_cid_policy == "prefer")
                    else 600
                ),
            )
            if from_number is None and state_match_active and state_cid_policy == "prefer":
                # Same-state numbers exist but are all busy; prefer-policy may
                # fall back to the whole pool rather than fail the call.
                logger.info(
            ...
            if from_number is None:
                if state_match_active and state_cid_policy == "strict":
                    # Same-state inventory exists but is fully in use: transient —
                    # return the run to the queue via pool-exhausted semantics.
                    logger.warning(
            ...
            initial_context = {
                **queued_run.context_variables,
                ...
                # HOPWHISTLE PATCH (state-matched caller ID): persist the
                # selection decision on the run for reporting/verification.
                "destination_state": dest_state,
        ...
        try:
            ...
            # Record channel -> workflow_run in Redis at DIAL time so the ARI
            # ChannelDestroyed handler can release the concurrency slot + rotated
            # caller-ID even when a call never answers (no StasisStart -> no
            # StasisEnd -> the normal release path never runs). Key matches
            # ari_manager _CHANNEL_KEY_PREFIX ("ari:channel:").
            try:
                if call_result.call_id:
                    _r = await rate_limiter._get_redis()
                    await _r.set(
                ...
                logger.warning(f"Failed to record channel->run mapping: {_map_err}")
    ...
    async def acquire_from_number(
        ...
        HOPWHISTLE PATCH: optional ``allowed_numbers`` restricts acquisition to a
        subset of the pool (state-matched caller-ID selection). An empty list
        short-circuits to None immediately.
