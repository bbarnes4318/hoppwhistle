...
class RateLimiter:
    ...
    async def acquire_from_number(
        ...
        HOPWHISTLE PATCH (state-matched caller ID): when ``allowed_numbers`` is
        provided, only members of that subset may be acquired — used to restrict
        selection to caller IDs whose area-code state matches the destination
        state. ``None`` (the default) preserves the original whole-pool behavior.
        An empty list acquires nothing and returns None.

        Returns the phone number if available, None if all (allowed) numbers are
        in use or the allowed subset is empty.
        """
        ...
        -- Optional allowed-subset filter (ARGV[3] = JSON array of numbers).
        -- Only members of the subset may be acquired.
        if ARGV[3] and ARGV[3] ~= '' then
            local allowed = cjson.decode(ARGV[3])
            local allowed_set = {}
            for i, n in ipairs(allowed) do
                allowed_set[n] = true
            end
            local filtered = {}
            for i, member in ipairs(available) do
                if allowed_set[member] then
        ...
        if allowed_numbers is not None:
            ...
            allowed_json = _json.dumps(sorted(set(allowed_numbers)))

        try:
            result = await redis_client.eval(
                lua_script, 1, key, now, stale_cutoff, allowed_json
            )
            if result:
